  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-16">
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: -20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20 }}
            ...
            style={{ transform: 'translateX(-50%)', background: 'var(--accent-sage)', color: myCharacter === 'toothless' ? '#080C14' : 'white' }}>
            {toast}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.header className="mb-10 text-center" initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="text-4xl mb-4">{myEmoji}</div>
        <h1 className="text-3xl md:text-4xl font-bold mb-5" style={{ color: 'var(--text-main)' }}>{scenario.name}</h1>
        <div className="p-6 rounded-2xl text-left" style={{ background: 'var(--accent-glow)', borderLeft: '4px solid var(--accent-sage)' }}>
          <p className="italic mb-3 text-base" style={{ color: 'var(--text-main)', opacity: 0.8 }}>{scenario.situation}</p>
      ...
      <div className="flex justify-between items-center">
        <Link href="/scenarios" className="font-bold transition-opacity hover:opacity-70" style={{ color: 'var(--text-muted)' }}>← Back</Link>
