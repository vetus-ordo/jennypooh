...
export default function Scenarios() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-14">
      ...
      <motion.div className="blueprint-card mb-10" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }}>
        <div className="flex flex-wrap justify-between items-end gap-4 mb-4">
          ...
          {firstUnanswered && completed > 0 && (
            <button onClick={() => router.push(`/scenarios/${firstUnanswered}`)}
      ...
      {Object.entries(categoryGroups).map(([catKey, cat], catIdx) => {
        ...
        return (
          <motion.section key={catKey} className="mb-12"
            initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + catIdx * 0.08 }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xl">{cat.emoji}</span>
                <span className="font-bold uppercase tracking-widest text-xs" style={{ color: 'var(--text-muted)' }}>{cat.label}</span>
              </div>
