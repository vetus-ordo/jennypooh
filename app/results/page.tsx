...
export default function Results() {
  ...
  const categoryScores = Object.entries(categoryGroups).map(([key, group]) => {
    ...
    return {
      ...group,
      score:    cats.length > 0 ? Math.round(cats.filter((r: any) => r.isMatch).length / cats.length * 100) : null,
            ...
            {countdown !== null ? (
              ...
              <motion.div key="score" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <p className="mt-4 text-sm" style={{ color: 'var(--text-muted)' }}>
                  {matches} matches out of {totalAnswered} answered
                </p>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {totalAnswered > 0 && (
        ...
        >
          <div className="p-8 text-center">
            ...
            <div className="text-xl font-bold mb-1" style={{ color: 'var(--text-main)' }}>
              You & {partnerName}
            </div>
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
              jennypooh · Life Scenarios
            </p>
          </div>
        </motion.div>
      )}
